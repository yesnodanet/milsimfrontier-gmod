...
function Clockwork:TranslateActivity(player, act)
	...
	local playerLibrary = Clockwork.player;
	if (playerLibrary and playerLibrary.GetWeaponRaised) then
		bIsRaised = playerLibrary:GetWeaponRaised(player, true);
	end;
...
if (SERVER) then
	...
	function Clockwork.kernel:IncludeSchema()
		if (CloudAuthX and CloudAuthX.kernel and CloudAuthX.kernel.IncludeSchema) then
			return CloudAuthX.kernel:IncludeSchema();
		end;

		local schemaFolder = self:GetSchemaFolder();

		return Clockwork.plugin:Include(schemaFolder.."/schema", true);
	end;
